    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                if (data) {
                    setProfile(data)
                    const status = await getInvestorStatus(data.id)
        ...
        <div className="max-w-3xl mx-auto space-y-8 animate-fadeIn">
            <header className="border-b border-white/5 pb-8">
                <h1 className="text-4xl font-rajdhani font-bold text-white uppercase tracking-wider flex items-center gap-4">
                    <User className="text-[#119dff]" size={40} />
                    My Profile
                </h1>
            ...
            <div className="bg-black/40 border border-white/10 rounded-2xl p-8 space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white outline-none focus:border-[#119dff]/50 transition-colors"
                        />
                    </div>
                    <div className="space-y-2">
                        <label className="text-xs uppercase tracking-widest text-white/40 flex items-center gap-2">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-[#119dff] font-bold uppercase tracking-wider text-sm">
                            {profile?.role}
                        </div>
                    </div>
                </div>
                ...
                <div className="border-t border-white/5 pt-8">
                    <h3 className="text-xl font-bold font-rajdhani text-white mb-6 flex items-center gap-2">
                        <Wallet className="text-[#119dff]" size={24} /> Connected Wallet
                    </h3>
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-xl p-6 flex flex-col md:flex-row items-center justify-between gap-6">
                        ...
                        <div className="flex flex-col items-center gap-2">
                            {/* ThirdWeb ConnectButton stubbed — install `thirdweb` to restore */}
                            <div className="px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-center">
                                <p className="text-white/40 text-xs">Install <code className="text-white/60">thirdweb</code> to enable wallet connection</p>
                            </div>
                ...
                <div className="border-t border-white/5 pt-8">
                    <h3 className="text-xl font-bold font-rajdhani text-white mb-6 flex items-center gap-2">
                        <Shield className="text-[#119dff]" size={24} /> Investor Accreditation
                    </h3>
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                        {investorStatus ? (
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <Link
                                    href="/nexus/onboarding"
                                    className="px-6 py-3 bg-[#119dff] hover:bg-[#119dff]/90 text-white rounded-lg transition-colors flex items-center gap-2"
                                >
                                    Get Started
                                    <ArrowRight size={18} />
                                </Link>
                ...
                <div className="border-t border-white/5 pt-8">
                    <button
                        onClick={handleSave}
                        disabled={saving}
                        className="px-8 py-4 bg-[#119dff] hover:bg-[#0d7acc] text-white font-bold rounded-xl flex items-center gap-2 transition-all shadow-lg shadow-red-900/20 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
