...
import { getCurrentProfile, getIntegrations, getSubsidiaries, getCampaigns, deleteCampaign } from '../../actions'
...
export default function VenturesPage() {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            ...
            try {
                ...
                // Mercury API not configured — silently skip
            }
        }
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            setLoading(true)
            try {
                const camps = await getCampaigns(activeTab)
                setCampaigns(camps || [])
            } catch (e) { console.error(e) }
            setLoading(false)
        }
    ...
    const LOGO_MAP: Record<string, string> = {
        network: '/BasaltHQ.png',
        basaltsurge: '/BasaltSurge.png',
        basaltvigil: '/BasaltVigil.png',
        basaltcrm: '/BasaltCRM.png',
        basalterp: '/BasaltERP.png',
        basaltcms: '/BasaltCMS.png',
        basaltecho: '/BasaltEcho.png',
        basaltonyx: '/BasaltOnyx.png',
    }
    const getLogo = (id: string) => LOGO_MAP[id] || '/BasaltHQ.png'
        ...
        <div className="flex h-[calc(100vh-100px)] animate-fadeIn overflow-hidden rounded-2xl border border-white/10 bg-[#0A0A0A] relative">
                    ...
                    {subsidiaries.map(sub => (
                        ...
                        >
                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center p-2 transition-all ${activeTab === sub.id ? 'bg-black/40 opacity-100' : 'opacity-50 group-hover:opacity-100'}`}>
                                <img src={getLogo(sub.id)} className="w-full h-full object-contain" />
                            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 relative scrollbar-thin scrollbar-thumb-white/10 w-full">
                ...
                <button
                    ...
                    className="md:hidden mb-6 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#119dff] border border-[#119dff]/20 px-4 py-2 rounded-lg bg-[#119dff]/5"
                >
                        ...
                        <div className="flex flex-col md:flex-row items-start justify-between gap-6 pb-8 border-b border-white/5">
                            <div className="flex items-center gap-6">
                                <div className="w-16 h-16 md:w-24 md:h-24 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center p-3 md:p-5 shadow-2xl shrink-0">
                                    <img src={getLogo(currentEntity.id)} className="w-full h-full object-contain" />
                                </div>
                            ...
                            <div className="flex bg-white/5 rounded-lg p-1 border border-white/10 self-start shrink-0 overflow-x-auto max-w-full">
                                {[
                                    ...
                                    ...(['admin', 'superadmin'].includes(role) ? [{ id: 'fundraising', icon: DollarSign, label: 'Fundraising' }] : []),
                                        ...
                                        <div className="space-y-6 relative z-10">
                                            ...
                                            <div className="grid grid-cols-2 gap-4 pt-6 border-t border-white/5">
                                                <div>
                                                    ...
                                                    <p className="text-2xl font-mono text-[#119dff]">{campaigns.filter(c => c.status === 'active').length}</p>
                                                </div>
                                                <div>
                        ...
                        {viewMode === 'invest' && (
                            <div className="space-y-8 animate-fadeIn">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {campaigns.map(round => (
                                        <div key={round.id} className="bg-white/[0.02] border border-white/10 rounded-xl p-6 hover:border-[#119dff]/30 transition-all group relative overflow-hidden">
                                            <div className="flex justify-between items-start mb-4">
                                                <span className={`px-2 py-0.5 text-[10px] uppercase font-bold rounded ${round.status === 'active' ? 'bg-[#119dff] text-white' : 'bg-white/10 text-white/40'}`}>
                                                    {round.status}
                                                </span>
                                            ...
                                            <h4 className="text-xl font-bold text-white mb-1 group-hover:text-[#119dff] transition-colors truncate">{round.name}</h4>
                                ...
                                <div className="space-y-6 flex-1">
                                    <div className="space-y-2">
                                        ...
                                        <div className="relative">
                                            ...
                                            <input
                                                type="number"
                                                className="w-full bg-black/40 border border-white/10 rounded-xl pl-10 pr-4 py-4 text-white text-lg font-mono outline-none focus:border-[#119dff]"
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-start gap-3 p-4 bg-[#119dff]/10 rounded-xl border border-[#119dff]/20">
                                            <Check className="text-[#119dff] mt-0.5" size={16} />
                                        ...
                                        <button
                                            ...
                                            className="w-full py-4 bg-[#119dff] disabled:opacity-50 hover:bg-[#0d7acc] text-white font-bold rounded-xl uppercase tracking-widest shadow-lg transition-all"
                                        >
                                            Generate Agreement
                                        </button>
                                    </div>
                            ...
                            {investStep === 'safe' && investmentAmount && (
                                <div className="absolute bottom-0 left-0 right-0 p-6 bg-white border-t border-gray-200 flex justify-between items-center shadow-[0_-10px_30px_rgba(0,0,0,0.1)]">
                                    ...
                                    <button
                                        ...
                                        disabled={!safeSigned}
                                        className="px-8 py-3 bg-[#119dff] disabled:bg-gray-300 text-white font-bold rounded-lg uppercase tracking-wider text-xs transition-all"
                                    >
                                        Counter-Sign & Fund
                                    </button>
                                </div>
