...
import { getOpenRounds } from '@/app/nexus/actions'
    ...
    useEffect(() => {
        async function load() {
            try {
                ...
                const data = await getOpenRounds()
                setRounds(data || [])
            } catch (e) {
                console.error(e)
            } finally {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {rounds.map((round, idx) => {
                        ...
                        return (
                            <div 
                                key={round._id}
                                className="bg-black/40 border border-white/10 rounded-2xl overflow-hidden hover:border-[#119dff]/30 transition-all group animate-in fade-in slide-in-from-bottom-8 fill-mode-both"
                                style={{ animationDelay: `${idx * 150}ms` }}
                            >
                                {/* Header */}
                                <div className="p-8 border-b border-white/10 relative overflow-hidden">
                                    <div className="absolute top-0 right-0 w-64 h-64 bg-[#119dff]/10 rounded-full blur-[80px] -mr-32 -mt-32 group-hover:bg-[#119dff]/20 transition-colors" />
                                ...
                                <div className="p-8 bg-black/20">
                                    <div className="flex justify-between items-end mb-3">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <p className="text-[#119dff] font-bold text-lg font-rajdhani">
                                    ...
                                    <div className="h-2 bg-black rounded-full overflow-hidden mb-8 border border-white/5">
                                        <div 
                                            className="h-full bg-gradient-to-r from-[#119dff] to-cyan-400 shadow-[0_0_10px_rgba(17,157,255,0.5)] transition-all duration-1000"
                                    ...
                                    <button
                                        ...
                                        className="w-full bg-[#119dff]/10 hover:bg-[#119dff]/20 border border-[#119dff]/30 text-[#119dff] hover:text-white font-bold py-4 rounded-xl transition-all flex items-center justify-center gap-2 group/btn"
                                    >
                                        Initiate Commitment 
