...
import { getSubsidiaries, getRecentDocuments, getDocuments } from '../../actions'
...
export default function DocumentsPage() {
    ...
    useEffect(() => {
        const fetchRecent = async () => {
            try {
                const data = await getRecentDocuments(5)
                setRecentDocs(data || [])
            } catch (e) { console.error(e) }
        }
        fetchRecent()
    ...
    useEffect(() => {
        ...
        const fetchDocs = async () => {
            setLoading(true)
            try {
                const data = await getDocuments(selectedFolder)
                setDocuments(data || [])
            } catch (e) { console.error(e) }
            setLoading(false)
        }
            ...
            <header className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 border-b border-white/5 pb-8 shrink-0">
                <div>
                    <h1 className="text-4xl font-rajdhani font-bold text-white uppercase tracking-wider flex items-center gap-4">
                        <FolderOpen className="text-[#119dff]" size={40} />
                        Global Data Room
                    </h1>
                ...
                <div className="flex gap-4">
                    <div className="flex bg-black/40 border border-white/10 rounded-xl p-1">
                        <button
                            ...
                            className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-[#119dff] text-white' : 'text-white/40 hover:text-white'}`}
                        >
                            <LayoutGrid size={18} />
                        </button>
                        <button
                            onClick={() => setViewMode('list')}
                            className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-[#119dff] text-white' : 'text-white/40 hover:text-white'}`}
                        >
                            <List size={18} />
                        </button>
                    </div>
            ...
            {selectedFolder ? (
                ...
                <div className="flex-1 bg-black/40 border border-white/10 rounded-2xl p-8 animate-fadeInUp">
                    <div className="flex items-center gap-4 mb-8">
                        ...
                        >
                            <img src={currentSubsidiary?.logo_url || '/BasaltHQShield.png'} alt={currentSubsidiary?.name} className="w-10 h-10 object-contain" />
                        </div>
                        <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        {documents.map(doc => (
                            <div key={doc.id} className="group p-4 bg-white/[0.02] border border-white/5 hover:border-[#119dff]/30 rounded-xl transition-all cursor-pointer hover:bg-white/[0.04]">
                                <div className="flex justify-between items-start mb-4">
                                    <FileText className="text-white/20 group-hover:text-white transition-colors" size={24} />
                                    <a href={doc.file_url} target="_blank" className="text-white/20 hover:text-[#119dff] transition-colors"><Download size={16} /></a>
                                </div>
                    ...
                    <div className="mt-12 p-4 rounded-lg bg-[#119dff]/5 border border-[#119dff]/10 flex items-center gap-3">
                        <Info className="text-[#119dff]" size={20} />
                        <p className="text-xs text-[#119dff]/80">
                ...
                <div className="flex-1 overflow-y-auto pb-10">
                        ...
                        {subsidiaries.map((sub, i) => (
                            ...
                            >
                                ...
                                <div className="flex justify-between items-start mb-6 relative">
                                    ...
                                    >
                                        <img src={sub.logo_url || '/BasaltHQShield.png'} alt={sub.name} className="w-full h-full object-contain drop-shadow-[0_0_5px_rgba(255,255,255,0.5)]" />
                                    </div>
                                    <div
                    ...
                    <div className="mt-12">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button className="text-xs text-[#119dff] uppercase tracking-wider hover:text-white transition-colors">View Audit Log</button>
                        </div>
                            ...
                            {recentDocs.map((doc, i) => (
                                <div key={i} className="flex items-center justify-between p-4 border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors group">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-sm font-bold text-white group-hover:text-[#119dff] transition-colors">{doc.title}</p>
