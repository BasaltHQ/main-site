...
import { getIntegrations, updateIntegrations } from '@/app/nexus/actions'
...
export default function SettingsPage() {
    ...
    const fetchSettings = async () => {
        ...
        try {
            ...
            setIntegrations(data.map((i: any) => ({ ...i, id: i._id })))
        } catch (e) { }
        setLoading(false)
    }
    ...
    const saveChanges = async () => {
        ...
        try {
            await updateIntegrations(integrations.map(i => ({ id: i.id, value: i.value })))
        ...
        <div className="space-y-8 animate-fadeIn max-w-5xl">
            <header className="flex justify-between items-end border-b border-white/5 pb-8">
                ...
                <button
                    onClick={saveChanges}
                    disabled={saving}
                    className="px-6 py-3 bg-[#119dff] hover:bg-[#0d7acc] disabled:bg-white/10 text-white font-bold rounded-xl flex items-center gap-2 transition-all shadow-lg shadow-red-900/20"
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div className="bg-black/40 border border-white/10 rounded-2xl p-8">
                        <h2 className="text-xl font-rajdhani font-bold text-white mb-6 flex items-center gap-3">
                            <Wallet className="text-[#119dff]" />
                            Treasury Wallets
                        </h2>

                        <div className="space-y-6">
                            {integrations.filter(i => i.category === 'wallet').map(wallet => (
                                <div key={wallet.id} className="bg-white/5 rounded-xl p-4 border border-white/5 focus-within:border-[#119dff]/50 transition-colors">
                    ...
                    <div className="bg-black/40 border border-white/10 rounded-2xl p-8">
                        ...
                        <div className="space-y-6">
                            {integrations.filter(i => i.category === 'api').map(api => (
                                <div key={api.id} className="bg-white/5 rounded-xl p-4 border border-white/5 focus-within:border-[#119dff]/50 transition-colors">
                ...
                <div className="space-y-8">
                    <div className="bg-gradient-to-br from-red-900/10 to-transparent border border-[#119dff]/20 rounded-2xl p-6">
                        <h3 className="text-lg font-rajdhani font-bold text-white mb-4 flex items-center gap-2">
                            <AlertCircle size={18} className="text-[#119dff]" />
                            Security Audit
                        </h3>
