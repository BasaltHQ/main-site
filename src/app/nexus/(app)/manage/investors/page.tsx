...
import { getProfiles, updateProfileStatus } from '../../../actions';
...
export default function InvestorCRMPage() {
    ...
    const fetchUsers = async () => {
        try {
            const data = await getProfiles('investor');
            setUsers(data || []);
            ...
            <header className="flex flex-col xl:flex-row xl:justify-between xl:items-end gap-6 border-b border-white/5 pb-8">
                <div>
                    <h1 className="text-4xl font-rajdhani font-bold text-white uppercase tracking-wider flex items-center gap-4">
                        <Users className="text-[#119dff]" size={40} />
                        Investor Relations
                    </h1>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="bg-black/40 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-sm text-white focus:border-[#119dff] outline-none w-full sm:w-64 transition-colors"
                        />
                    </div>
                        ...
                        {['all', 'pending', 'approved', 'rejected'].map((f) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-xs uppercase tracking-wider font-bold transition-all ${filter === f
                                    ? 'bg-[#119dff] text-white shadow-lg'
                    ...
                    <div className="space-y-3 animate-fadeInUp">
                        {filteredUsers.map((user) => (
                            <Link
                                ...
                                className="block"
                            >
                                <div className="bg-black/40 backdrop-blur-sm border border-white/10 rounded-xl p-4 flex flex-col lg:flex-row items-center gap-6 group hover:border-[#119dff]/30 hover:bg-white/[0.02] transition-all cursor-pointer">
                                    {/* Avatar & Basic Info */}
                                    <div className="flex items-center gap-4 flex-1 w-full lg:w-auto">
                                        <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-[#119dff]/10 to-orange-600/10 flex items-center justify-center text-[#119dff] font-bold text-lg border border-[#119dff]/20 group-hover:scale-105 transition-transform">
                                            {user.full_name?.charAt(0) || user.email?.charAt(0)}
                                        </div>
                                        <div className="overflow-hidden min-w-0">
                                            <div className="flex items-center gap-3">
                                                <h3 className="text-lg font-bold font-rajdhani text-white truncate group-hover:text-[#119dff] transition-colors">{user.full_name || 'Unknown User'}</h3>
                                                {user.company_name && (
