...
export default function ProposalWizard({ subsidiaryId }: Props) {
    ...
    async function fetchCapTable() {
        try {
            const res = await fetch(`/api/nexus/cap-table?subsidiaryId=${subsidiaryId}`)
            const data = await res.json()

            if (data.entries) {
                const stakeholders: Stakeholder[] = data.entries.map((entry: any) => ({
    ...
    async function fetchGovernanceRules() {
        try {
            const res = await fetch(`/api/nexus/governance?subsidiaryId=${subsidiaryId}`)
            const data = await res.json()
            if (data.rules) {
                setGovernanceRules(data.rules.map((r: any) => ({
                    id: r._id || r.id,
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-8">
                ...
                <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                    <div
                        className="h-full bg-[#119dff] transition-all"
            ...
            <div className="bg-black/40 border border-white/10 rounded-2xl p-8">
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white focus:border-[#119dff] focus:outline-none"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                className="w-full h-24 px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white focus:border-[#119dff] focus:outline-none resize-none"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                className="w-full h-32 px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white focus:border-[#119dff] focus:outline-none resize-none"
                            />
                        </div>
                    </div>
                )}
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <button
                                    onClick={addInvestor}
                                    className="px-4 py-2 bg-[#119dff] hover:bg-[#119dff]/90 text-white rounded-lg text-sm flex items-center gap-2"
                                >
                                    <Plus size={16} />
                                    Add Investor
                                </button>
                                ...
                                {newInvestors.map((investor, idx) => (
                                    <div key={idx} className="flex gap-3">
                                        <input
                                            ...
                                            className="flex-1 px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white focus:border-[#119dff] focus:outline-none"
                                        />
                                        <input
                                            type="number"
                                            ...
                                            placeholder="Amount"
                                            className="w-40 px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white focus:border-[#119dff] focus:outline-none"
                                        />
                                        <button
                ...
                {step === 3 && dilutionImpact && (
                    <div className="space-y-6">
                        <div className="flex items-center gap-3 mb-6">
                            <TrendingDown className="text-[#119dff]" size={32} />
                ...
                {step === 4 && (
                    <div className="space-y-6">
                        <div className="flex items-center gap-3 mb-6">
                            <Settings className="text-[#119dff]" size={32} />
                                    ...
                                    .map(rule => (
                                        ...
                                        >
                                            <div className="flex justify-between items-start mb-2">
                                                ...
                                                <span className="text-[#119dff] font-bold text-sm">{rule.approvalThreshold}%</span>
                                            </div>
                        ...
                        <label className="flex items-center gap-3 p-4 bg-white/5 rounded-lg cursor-pointer">
                            <input
                                ...
                                className="w-5 h-5 accent-[#119dff]"
                            />
                            <div>
                ...
                {step === 5 && (
                    <div className="space-y-6">
                        ...
                        <div className="p-4 bg-gradient-to-r from-[#119dff]/10 to-purple-500/10 border border-[#119dff]/30 rounded-lg flex items-start gap-3">
                            <AlertTriangle className="text-[#119dff] flex-shrink-0 mt-1" size={24} />
                            <div>
                ...
                <div className="flex justify-between mt-8 pt-6 border-t border-white/10">
                    ...
                    {step < totalSteps && (
                        <button
                            onClick={() => setStep(step + 1)}
                            disabled={!canProceed()}
                            className="px-8 py-3 bg-[#119dff] hover:bg-[#119dff]/90 disabled:bg-white/10 disabled:text-white/40 text-white font-bold rounded-lg ml-auto flex items-center gap-2"
                        >
                            Continue
                            <ArrowRight size={20} />
                        </button>
