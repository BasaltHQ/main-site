...
export default function GovernanceSettings({ subsidiaryId }: Props) {
    ...
    async function handleSave() {
        ...
        try {
            const body: any = {
                subsidiary_id: editing.subsidiaryId,
                rule_type: editing.ruleType,
                title: editing.title,
                description: editing.description,
                requires_approval: editing.requiresApproval,
                approval_threshold: editing.approvalThreshold,
                vote_weight_type: editing.voteWeightType,
                eligible_voters: editing.eligibleVoters,
                voting_period_days: editing.votingPeriodDays,
                notice_period_days: editing.noticePeriodDays,
                founder_veto: editing.founderVeto,
                board_approval_required: editing.boardApprovalRequired,
                requires_unanimous: editing.requiresUnanimous,
                exemptions: editing.exemptions,
                is_active: true,
            }
            if (editing.id) body.id = editing.id

            const res = await fetch('/api/nexus/governance', {
                method: 'POST',
    ...
    async function handleDelete(ruleId: string) {
        ...
        try {
            await fetch(`/api/nexus/governance?id=${ruleId}`, { method: 'DELETE' })
            await fetchRules()
        } catch (error) {
                            ...
                            {rulesByType[type].length === 0 && (
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        ...
                                        className="px-4 py-2 bg-[#119dff] hover:bg-[#119dff]/90 text-white text-sm rounded-lg flex items-center gap-2"
                                    >
                                        <Plus size={16} />
                                        Create Rule
                                    </button>
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full h-20 px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white focus:border-[#119dff] focus:outline-none resize-none"
                                />
                            </div>
                            ...
                            <div className="flex gap-4 pt-6 border-t border-white/10">
                                ...
                                <button
                                    onClick={handleSave}
                                    className="flex-1 px-6 py-3 bg-[#119dff] hover:bg-[#119dff]/90 text-white font-bold rounded-lg flex items-center justify-center gap-2"
                                >
                                    <CheckCircle size={20} />
                                    Save Rule
                                </button>
