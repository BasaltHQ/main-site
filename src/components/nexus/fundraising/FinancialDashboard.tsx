...
export default function FinancialDashboard({ subsidiaryId }: { subsidiaryId: string }) {
    ...
    async function fetchData() {
        setLoading(true)
        try {
            const res = await fetch(`/api/nexus/financials?subsidiaryId=${subsidiaryId}`)
            const data = await res.json()

            if (data.snapshot) {
                setSnapshot({
                    periodStart: data.snapshot.period_start,
                    periodEnd: data.snapshot.period_end,
                    totalRevenue: data.snapshot.total_revenue || 0,
                    mrr: data.snapshot.mrr || 0,
                    arr: data.snapshot.arr || 0,
                    revenueGrowthRate: data.snapshot.revenue_growth_rate || 0,
                    totalExpenses: data.snapshot.total_expenses || 0,
                    cashBalance: data.snapshot.cash_balance || 0,
                    monthlyBurnRate: data.snapshot.monthly_burn_rate || 0,
                    runwayMonths: data.snapshot.runway_months || 0,
                    expenseRunwayMonths: data.snapshot.expense_runway_months || ((data.snapshot.total_expenses || 0) > 0 ? (data.snapshot.cash_balance || 0) / (data.snapshot.total_expenses || 1) : 0),
                    totalUsers: data.snapshot.total_users || 0,
                    payingCustomers: data.snapshot.paying_customers || 0,
                    cac: data.snapshot.cac || 0,
                    ltv: data.snapshot.ltv || 0,
                    ltvCacRatio: data.snapshot.ltv_cac_ratio || 0,
                    ruleOf40: data.snapshot.rule_of_40 || 0,
                })
            } else {
                setSnapshot(null)
            }
        ...
        <div className="space-y-8 relative">
            ...
            {snapshot && (
                <>
                    ...
                    <div className="bg-black/40 border border-white/10 rounded-2xl p-8">
                        <h2 className="text-2xl font-bold text-white mb-6 font-rajdhani flex items-center gap-3">
                            <BarChart3 className="text-[#119dff]" size={32} />
                            Unit Economics
                        </h2>
                        ...
                        <div className="grid grid-cols-3 gap-8">
                            ...
                            <div className="text-center">
                                <div className="bg-gradient-to-br from-[#119dff]/20 to-purple-500/20 border border-[#119dff]/30 rounded-xl p-8 mb-4">
