...
import { getBankAccounts } from '@/app/nexus/actions';
...
export default function WireInstructions() {
    ...
    useEffect(() => {
        const fetchAccounts = async () => {
            ...
            // 2. Fallback to Supabase (now Mongo via Actions) DB if API fails or is empty
            try {
    ...
    return (
        <div className="space-y-6">
            {accounts.map((account) => (
                <div key={account.id} className="p-6 rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.02] to-transparent relative group">
                    ...
                    <h3 className="text-sm font-bold text-white/80 uppercase tracking-widest mb-6 flex items-center gap-2">
                        <Building size={16} className="text-[#119dff]" />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => handleCopy(account.beneficiary_name, `name-${account.id}`)} className="text-white/20 hover:text-[#119dff] transition-colors">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => handleCopy(account.account_number, `acc-${account.id}`)} className="text-white/20 hover:text-[#119dff] transition-colors">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => handleCopy(account.routing_number, `rout-${account.id}`)} className="text-white/20 hover:text-[#119dff] transition-colors">
                        ...
                        {account.swift_code && (
                            <div>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={() => handleCopy(account.swift_code, `swift-${account.id}`)} className="text-white/20 hover:text-[#119dff] transition-colors">
            ...
            <p className="text-xs text-center text-white/30 italic">
                * Please include your BasaltHQ Investor ID in the wire memo for faster processing.
            </p>
        </div>
    );
}
