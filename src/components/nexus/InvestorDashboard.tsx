...
import { getInvestorCampaigns } from '@/app/nexus/actions'
...
export default function InvestorDashboard({ profile }: { profile: any }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await getInvestorCampaigns()
                setCampaigns(data || [])
            } catch (err) {
                console.error(err)
            } finally {
    ...
    return (
        <div className="space-y-8 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Main Value Card */}
                <div className="lg:col-span-2 bg-gradient-to-br from-red-900/20 to-orange-900/20 border border-[#119dff]/20 rounded-2xl p-8 relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-64 h-64 bg-[#119dff]/10 blur-[100px] rounded-full -mr-32 -mt-32" />

                    <h2 className="text-white/60 uppercase tracking-widest text-xs font-bold mb-2 flex items-center gap-2">
                        <Activity size={14} className="text-[#119dff]" />
                        Total Asset Value
                    </h2>
                    ...
                    <div className="mt-8 grid grid-cols-2 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <p className="text-xl font-rajdhani font-semibold text-[#119dff]">--%</p>
                        </div>
                        <div>
                ...
                <div className="bg-white/5 border border-white/5 rounded-2xl p-8 flex flex-col justify-between hover:border-[#119dff]/30 transition-colors group">
                    <div>
                    ...
                    <Link href="mailto:ir@basalthq.com" className="w-full py-3 bg-white/5 hover:bg-[#119dff]/10 border border-white/10 hover:border-[#119dff]/50 text-white hover:text-[#119dff] rounded-lg transition-all text-xs uppercase tracking-wider font-bold flex items-center justify-center gap-2">
                        Contact IR Team
                    </Link>
                </div>
            </div>
                ...
                <div className="bg-black/40 border border-white/10 rounded-2xl p-6 flex flex-col">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-lg font-bold font-rajdhani text-white flex items-center gap-2">
                            <FileText className="text-[#119dff]" size={20} />
                            Recent Documents
                        </h3>
                        <Link href="/nexus/documents" className="text-xs text-[#119dff] hover:text-white uppercase tracking-wide transition-colors">
                            View Console
                        </Link>
                    </div>
                    ...
                    <div className="space-y-4">
                        {loading ? (
                            ...
                            campaigns.map(campaign => (
                                <div key={campaign.id} className="p-4 bg-gradient-to-r from-red-900/5 to-transparent border border-[#119dff]/10 rounded-xl group hover:border-[#119dff]/40 transition-all cursor-pointer hover:bg-white/[0.02]">
                                    <div className="flex justify-between items-start mb-2">
                                        <div>
                                            <h4 className="font-bold text-white group-hover:text-[#119dff] transition-colors font-rajdhani">{campaign.name}</h4>
                                            <p className="text-[10px] text-white/40 uppercase tracking-wider mt-0.5">
                                                Target: {formatUSD(campaign.target_amount)}
                                            </p>
                                        </div>
                                        <span className="text-[10px] px-2 py-1 bg-[#119dff]/20 text-[#119dff] rounded-full uppercase tracking-wider font-bold">Live</span>
                                    </div>
                                    ...
                                    <div className="mt-4 flex items-center justify-between">
                                        ...
                                        <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-[#119dff] group-hover:text-white transition-all">
                                            <ArrowRight size={14} />
                                        </div>
                                    </div>
                                </div>
